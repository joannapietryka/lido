...
type ApartmentUnitPhotosModalProps = {
  unitNumber: string
  photos: string[]
  index: number
  onClose: () => void
...
export function ApartmentUnitPhotosModal({
  unitNumber,
  photos,
  index,
  onClose,
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t('apartmentDetail.availability.photosLabel', { number: unitNumber })}
      onClick={onClose}
    >
      <button
        type="button"
      ...
      <div className="relative max-w-5xl w-full max-h-[85vh]" onClick={(e) => e.stopPropagation()}>
        <p className="text-white text-center text-lg font-medium mb-4">
          {t('apartmentDetail.availability.photosTitle', { number: unitNumber })}
        </p>
        <img
          src={photo}
          alt={t('apartmentDetail.availability.photoAlt', { number: unitNumber, index: index + 1 })}
